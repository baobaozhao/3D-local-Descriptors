 function Hist_out=LD(pointcloud,Key_indices,LPAs,R,deviation_angle_size,LRAs)
...
%
hist_size=deviation_angle_size;
...
[ad_index,Dists]=rangesearch(pointcloud,pointcloud(Key_indices,:),R);
...
%         radial_list=ceil(Dist(j)/radial_step);
...
%         end
        ...
        index=deviation_angle_list;
        
        %index2=(projected_radial_list-1)*deviation_angle_size+deviation_angle_list;
        
        hist(index)=hist(index)+1;
        %hist(LL1+index2)=hist(LL1+index2)+1;
...
% Num=floor(radial_size/2);
% Index_eliminating=[];
% for i=1:Num
%     L_r=R-sqrt(R^2-(R-i*radial_step)^2);
...
%             Index_eliminating=[Index_eliminating;radial_size-i+1,projected_radial_size-j+1];
%         end
%     end
% end
