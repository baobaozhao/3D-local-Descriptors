function Hist_out=radial_elevation_ld(pointcloud,Key_indices,LPAs,R,radial_size,elevation_angle_size,deviation_angle_size,LRFs)
...
%  output:Hist_out - the vectors of radial_elevation_ld descriptor generated on all key pointcloud.


L_key_points=length(Key_indices);
hist_size=radial_size*elevation_angle_size*deviation_angle_size;
...
for i=1:L_key_points
    ...
    for j=1:neigh_size
       ...
       % radial_list=ceil((local_points(j,1)+R)/X_step);
        %XY_angle_list=ceil((local_points(j,2)+R)/Y_step);
...
%         elevation_angle_list
        ...
        index=(radial_list-1)*elevation_angle_size*deviation_angle_size+(elevation_angle_list-1)*deviation_angle_size+deviation_angle_list;
