function Hist_out=radial_azimuth_elevation_ld(pointcloud,Key_indices,LPAs,R,radial_size,azimuth_size,elevation_size,deviation_angle_size,LRFs)
...
%         elevation_size - the dimension along elevation direction.
%         deviation_angle_size - the dimension of deviation angle between LPA and LRA.
%         LRFs - the LRF Attached to each key key_point.

%  output:Hist_out - the vectors of radial_azimuth_elevation_ld descriptor generated on all key pointcloud.


L_key_points=length(Key_indices);
hist_size=radial_size*azimuth_size*elevation_size*deviation_angle_size;

Hist=zeros(L_key_points,hist_size);

deviation_angle_step=180/deviation_angle_size;

%X_step=R/radial_size;   % Çò¾¶Ïò¾àÀë¡£
...
azimuth_step=180/azimuth_size; 

elevation_step=zeros(elevation_size,1);
for i=1:elevation_size
    
    %begin_degree=1-(i-1)*zhi;
    end_degree=1-i*(2/elevation_size);
    elevation_step(i)=acos(end_degree)*180/pi;
...
for i=1:L_key_points
    ...
    for j=1:neigh_size
        ...
        azimuth_list=find_y_angle_list(local_points(j,:),azimuth_step);
        
        if dists(j)~=0
        z_angle=180*acos(local_points(j,3)/dists(j))/pi;
        elevation_list=judge_circle_location(elevation_step,z_angle);
        else
            elevation_list=1;
        end
        ...
        if azimuth_list>azimuth_size
            azimuth_list=azimuth_size;
        end
        ...
        if elevation_list>elevation_size
            elevation_list=elevation_size;
        end
        
        
        index=(radial_list-1)*azimuth_size*elevation_size*deviation_angle_size+(azimuth_list-1)*elevation_size*deviation_angle_size+(elevation_list-1)*deviation_angle_size+deviation_angle_list;
