 function Hist_out=XYZ_ld(pointcloud,Key_indices,LPAs,R,X_size,Y_size,Z_size,deviation_angle_size,LRFs)
...
%  output:Hist_out - the vectors of XYZ_ld descriptor generated on all key pointcloud.

L_key_points=length(Key_indices);
hist_size=deviation_angle_size*X_size*Y_size*Z_size;
...
for i=1:L_key_points
    ...
    %Normal_length(j)=local_points(j,:)*LRA';
        ...
        index=(X_list-1)*Y_size*Z_size*deviation_angle_size+(Y_list-1)*Z_size*deviation_angle_size+(Z_list-1)*deviation_angle_size+deviation_angle_list;
