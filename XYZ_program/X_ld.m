 function Hist_out=X_ld(pointcloud,Key_indices,LPAs,R,X_size,deviation_angle_size,LRFs)
...
%         X_size - the dimension of X direction.
%         deviation_angle_size - the dimension of deviation angle between LPA and LRA.
%         LRAs - the LRA Attached to each key key_point.

%  output:Hist_out - the vectors of X_ld descriptor generated on all key pointcloud.
 
 
L_key_points=length(Key_indices);
hist_size=deviation_angle_size*X_size;
...
for i=1:L_key_points
    ...
    %Normal_length(j)=local_points(j,:)*LRA';
        ...
        index=(X_list-1)*deviation_angle_size+deviation_angle_list;
