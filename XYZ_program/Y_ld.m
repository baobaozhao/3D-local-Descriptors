 function Hist_out=Y_ld(pointcloud,Key_indices,LPAs,R,Y_size,deviation_angle_size,LRFs)
...
%         Y_size - the dimension of Y direction.
%         deviation_angle_size - the dimension of deviation angle between LPA and LRA.
%         LRAs - the LRA Attached to each key key_point.

%  output:Hist_out - the vectors of Y_ld descriptor generated on all key pointcloud.

L_key_points=length(Key_indices);
hist_size=deviation_angle_size*Y_size;
...
for i=1:L_key_points
    ...
    %Normal_length(j)=local_points(j,:)*LRA';
        ...
        index=(Y_list-1)*deviation_angle_size+deviation_angle_list;
